...
//{
//    QMenu menu;
//    menu.addAction(tr("Новая &вкладка"), this, &TabWidget::createTab, QKeySequence::AddTab);
//    int index = tabBar()->tabAt(pos);
//    if (index != -1) {
//        QAction *action; //= menu.addAction(tr("Копировать вкладку"));
//        connect(action, &QAction::triggered, this, [this,index]() {
//            cloneTab(index);
//        });
//        menu.addSeparator();
//        action = menu.addAction(tr("Закрыть &другиеr вкладки"));
//        connect(action, &QAction::triggered, this, [this,index]() {
//            closeOtherTabs(index);
//        });
//        menu.addSeparator();
//        action = menu.addAction(tr("Перезагрузить вкладку"));
...
//        menu.addSeparator();
//    }
//    menu.addAction(tr("Перезагрузить все вкладки"), this, &TabWidget::reloadAllTabs);
...
void TabWidget::setupView(WebView *webView)
{
    ...
    connect(webView, &WebView::forcedTitleSet, [this, webView](const QString &title) {
...
//        static const QStringList ext_b{".htm", "html"};
//        if (ext_b.contains(title.right(4)))
//            return;
//        int index = indexOf(webView);
//        if (index != -1) {
//            setTabText(index, title);
...
WebView *TabWidget::createBackgroundTab()
{
    ...
    setupView(webView);
    int index = addTab(webView,
                       tr("(Новая)"));
//    if(index>0) //Контекстное меню не на всех вкладках
//        webView->setContextMenuPolicy(Qt::NoContextMenu);
    //NOTE: так же можно отключать контекстное меню через  JavaScript на странице. Это было бы более правильно. <body oncontextmenu="return false">. Для отдельных элементов - по другому. Пока оставляем этот способ, т.к. тупо быстрее.
